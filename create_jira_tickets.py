...
def form_rows(row_dict, index_col_map=dev_index_col_map):
    ...
    for uuid, data in row_dict.items():
        ...
        if data['Parent Ticket'] and \
                data['Parent Issue Type'] in ("Epic, Task"):
            ticket = data['Jira Ticket']
        ...
        # Remove the row from the dict after it's been transformed into a
        # Smartsheet object.
        loop_dict.pop(uuid)
        index_rows_to_add.append(new_row)
    return index_rows_to_add, loop_dict
...
def loop():
    ...
    if parent:
        ...
        rows_to_write, looped_child = form_rows(child)
        write_to_index(rows_to_write, dev_index_sheet)
        sleep(600)
        loop()
