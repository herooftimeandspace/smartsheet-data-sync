...
def test_build_linked_cell_0(index_sheet_fixture, sheet_fixture):
    import data_module.build_data as build_data
    index_sheet, index_col_map, _, index_row = index_sheet_fixture
    idx_row_id = index_row.id
    _, dest_col_map, _, _ = sheet_fixture
    with pytest.raises(TypeError):
        build_data.build_linked_cell("jira_index_sheet", index_col_map,
                                     dest_col_map, idx_row_id,
                                     app_vars.jira_col)
    with pytest.raises(TypeError):
        build_data.build_linked_cell(index_sheet, "jira_index_col_map",
                                     dest_col_map, idx_row_id,
                                     app_vars.jira_col)
    with pytest.raises(TypeError):
        build_data.build_linked_cell(index_sheet, index_col_map,
                                     "dest_col_map", idx_row_id,
                                     app_vars.jira_col)
    with pytest.raises(TypeError):
        build_data.build_linked_cell(index_sheet, index_col_map,
    ...
    with pytest.raises(ValueError):
        build_data.build_linked_cell(index_sheet, {},
                                     dest_col_map, idx_row_id,
                                     app_vars.jira_col)
    with pytest.raises(ValueError):
        build_data.build_linked_cell(index_sheet, index_col_map,
...
def test_build_linked_cell_1(index_sheet_fixture, sheet_fixture):
    import data_module.build_data as build_data
    index_sheet, index_col_map, _, index_row = index_sheet_fixture
    idx_row_id = index_row.id
    _, dest_col_map, _, _ = sheet_fixture
    link_cell = build_data.build_linked_cell(index_sheet,
                                             index_col_map, dest_col_map,
...
def test_build_row_0(row_fixture, columns_to_link, sheet_fixture,
    ...
    with pytest.raises(TypeError):
        build_data.build_row("row", columns_to_link, dest_col_map,
                             index_sheet, index_col_map, idx_row_id)
    with pytest.raises(TypeError):
        build_data.build_row(row, "columns_to_link", dest_col_map,
                             index_sheet, index_col_map, idx_row_id)
    with pytest.raises(TypeError):
    ...
    with pytest.raises(TypeError):
        build_data.build_row(row, columns_to_link, dest_col_map,
                             "jira_index_sheet", index_col_map,
                             idx_row_id)
    with pytest.raises(TypeError):
        build_data.build_row(row, columns_to_link, dest_col_map,
                             index_sheet, "jira_index_col_map",
                             idx_row_id)
    with pytest.raises(TypeError):
        build_data.build_row(row, columns_to_link, dest_col_map,
...
# import data_module.build_data as module_0
...
#         assert module_0.logger.name == 'data_module.build_data'
